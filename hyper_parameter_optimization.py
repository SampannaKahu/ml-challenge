...
logging.basicConfig(filename='./hyper_parameter_optimization.log', level=logging.DEBUG)
...
class MultiColumnLabelEncoderV2:
    def __init__(self, columns=None):
        if columns is None:
            raise ValueError('Please pass column names to the MultiColumnLabelEncoder')
        self.columns = columns  # array of column names to encode
        self.encoder_list = []
    ...
    def fit(self, X, y=None):
        ...
        i = 0
        for col in self.columns:
            encoder_list.append(LabelEncoder().fit(X[col]))
            i += 1
    ...
    def transform(self, X):
        ...
        i = 0
        for col in self.columns:
            output[col] = self.encoder_list[i].transform(output[col])
...
class Binning(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        for idx in range(0, len(self.cols)):
            X[:, idx] = self.my_funct(arr=X[:, idx])
            # X[:, idx] = [i for i in np.digitize(X[:, idx], self.bins)]
        return X

    def my_funct(self, arr):
        return np.ceil(100 * arr) / 100
...
class AdvancedOneHotEncoder(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        transform_dummies = pd.get_dummies(X)
        transform_columns = transform_dummies.columns
        cols_to_drop = [col for col in transform_columns if col not in self.fit_columns]
        cols_to_add = [col for col in self.fit_columns if col not in transform_columns]
        ...
        return transform_dummies.loc[transform_dummies.sum(axis=1) != 0]

    def fit(self, X, y=None):
        fit_dummies = pd.get_dummies(X)
        self.fit_columns = fit_dummies.columns
        return self
...
clf2 = RandomForestClassifier(n_jobs=-1, n_estimators=200, max_depth=13, random_state=42, min_samples_split=6,
                              min_impurity_split=0.05, bootstrap=False, verbose=10)
...
mlp = MLPClassifier(hidden_layer_sizes=(100, 100, 100), verbose=10)

clf = RandomForestClassifier(n_jobs=-1, n_estimators=200, max_depth=19, random_state=42,
                             min_samples_split=7,
                             min_impurity_split=0.04, bootstrap=False)
# --------------------Constructing the pipelines-----------
# bins1 = np.linspace(start=0, stop=1, num=101, endpoint=True)
bins1 = [0.01 * i for i in range(0, 100)]
numerical_feature_extractor = Pipeline([
    ...
    # ('binning', Binning(normal_numerical_feature_columns, bins1))
    # ('df_to_matrix', ConvertDFToMatrix())
])

# bins2 = np.linspace(start=0, stop=1, num=101, endpoint=True)
bins2 = [0.01 * i for i in range(0, 100)]
...
categorical_feature_extractor = Pipeline([
    ...
    # ('adv_onehot', AdvancedOneHotEncoder())
])
...
feature_selector = SelectPercentile(score_func=f_classif, percentile=70)
...
pipeline_score = final_pipeline.score(df_test_X, actual_Y['test'])
print 'Pipeline Score: ', pipeline_score
logging.info('Pipeline Score: ' + str(pipeline_score))

pred_prob_Y = {}
pred_prob_Y['test'] = final_pipeline.predict_proba(df_test_X)[:, 1]
roc_auc_score = metrics.roc_auc_score(actual_Y['test'], pred_prob_Y['test'])
print 'ROC_AUC Score: ', roc_auc_score
logging.info('ROC_AUC Score: ' + str(roc_auc_score))
